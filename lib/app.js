...
const valueline = d3.line()
      .x(d => x(d.key))
      .y(d => y(d.value.Playoff))

const valueline2 = d3.line()
      .x(d => x(d.key))
      .y(d => y(d.value.Season))
...
dataset.then(data => {
  data = d3.nest()
    ...
    .rollup(v => {  
      return {
        Playoff: ((d3.sum(v, d => d.serieshr2count) + d3.sum(v, d => d.serieshr1count)) / (d3.sum(v, d => d.gamesInSeries) * 2)),
  ...
  const path = svg.append("path") //playoffsLine
    .data([data])
      .attr("class", "line")
      .attr("d", valueline)  
