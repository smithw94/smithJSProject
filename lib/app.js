...
function makeBar(year) {
  ...
  dataset.then(data => {
    ...
    x0.domain(filtered[0].value.map(d => d.series))
    ...
    svg.append("g") // yaxis
      ...
      svg.append("text")
        .attr("class", "ylabel")
        .attr("transform", "rotate(-90) translate(-230, -250)")
        // .attr("y", 50)
    ...
    seriesName.selectAll("t1image") //Team1 Name
      .data(d => d.value)
      .enter()
      .append("svg:image")
      .attr('xlink:href', d => `./assets/icons/${d.t1Name}.png`)
      .attr("transform", d => "translate(" + x0(d.series) + ", -10)")
      .attr("x", d => x1('t1Val') - 5)
      .attr("y", d => ((height/2) - d.t1Val) > (height/2) ? (height/2) - 50 : ((height/2) - d.t1Val) - 50)

    seriesName.selectAll("t2image") //Team2Name
      .data(d => d.value)
      .enter()
      .append("svg:image")
      .attr('xlink:href', d => `./assets/icons/${d.t2Name}.png`)
      .attr("transform", d =>  "translate(" + x0(d.series) + ", 0)")
      .attr("x", d => x1('t2Val') )
