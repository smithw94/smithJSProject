  ...
  dataset.then(data => {
    ...
    svg.append("g") // yaxis
      ...
      svg.append("text")
        .attr("transform", "translate(" + -1*margin.left + "," + -1*margin.top/2 + ")")
        .style("text-anchor", "start")
        .text("Change in HRA per team per series");
    ...
    seriesName.selectAll("text") //Team1 Name
      ...
      .text(d => d.t1Name)
      .attr("transform", d => "translate(" + x0(d.series) + ",0)")
      .attr("x", d => x1('t1Val')-15)
      .attr("y", d => ((height/2) - d.t1Val) > (height/2) ? (height/2) - 5 : ((height/2) - d.t1Val) - 5)
    ...
    seriesName.selectAll("t2Name") //Team2Name
      ...
      .attr("transform", d =>  "translate(" + x0(d.series) + ", 0) ")
      .attr("x", d => x1('t2Val') )
      .attr("y", d => ((height/2) - d.t2Val) > (height/2) ? ((height/2) + Math.abs(d.t2Val) + 10) : ((height/2) + 10))
